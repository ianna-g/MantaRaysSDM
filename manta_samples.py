...
Usage:
  ...
  2) python mmf_enrich_samples.py
"""
...
# Max distance (km) to accept a CO-OPS station (currents/tides)
MAX_COOPS_DIST_KM = 150.0   # increased to ensure we find tide stations

# Thread workers for batched fetches (keep modest to avoid rate limits)
WORKERS_GHCND = 6
WORKERS_COOPS = 6
...
_TIDE_DAY_SERIES: Dict[Tuple[str, datetime], Tuple[np.ndarray, np.ndarray]] = {}
...
def nearest_coops_station(lat: float, lon: float, stype: str) -> Optional[Dict]:
    ...
    for s in table:
        try:
            ...
            continue
        d = float(haversine_km(lat, lon, slat, slon))
        if d < best_d:
            best, best_d = s, d
...
def find_nearest_ghcnd_station(lat: float, lon: float, bbox_size: float = 0.5) -> Optional[Dict]:
    ...
    for mult in [1, 2, 4, 8]:
        ...
        best = min(stations, key=lambda s: float(haversine_km(lat, lon, s["latitude"], s["longitude"])))
        break
...
def ghcnd_daily(station_id: str, date: datetime) -> Tuple[Optional[float], Optional[float]]:
    key = (station_id, date)
    if key in _GHCND_DAY_CACHE:
        return _GHCND_DAY_CACHE[key]
    ...
    try:
        ...
        pass
    _GHCND_DAY_CACHE[key] = (tavg_c, prcp_mm)
...
def coops_currents_obs_daily_mean_north(station_id: str, date: datetime) -> Optional[float]:
    ...
    try:
        ...
        for r in data:
            try:
                v = float(r["Speed"])
                deg = float(r["Direction"])      # direction TOWARD; 0°=north
...
# --------------------------- Tides per (station, day) ------------------------
def fetch_tide_day_series(station_id: str, date: datetime) -> Tuple[np.ndarray, np.ndarray]:
    """
    Fetch 6-min predicted tide for the UTC day with datum=MLLW.
    Fallback to observed water_level if predictions unavailable.
    Returns (timestamps_sec_utc, heights_m). Empty arrays if nothing.
    """
    key = (station_id, date)
    ...
    p_pred = {
        ...
        "format": "json", "station": station_id,
    }
    try:
        js = _coops_get(COOPS_DATA, p_pred)
        if "error" not in js:
            preds = js.get("predictions", []) or js.get("data", [])
            if preds:
                arr_t = np.array([
                    datetime.strptime(p["t"], "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc).timestamp()
                    for p in preds if p.get("t") is not None
                ], dtype=float)
                arr_v = np.array([float(p["v"]) for p in preds if p.get("v") is not None], dtype=float)
                _TIDE_DAY_SERIES[key] = (arr_t, arr_v)
                return arr_t, arr_v
    except Exception:
        pass

    # Fallback: observed water level
    p_obs = {
        "product": "water_level",
        "application": "mmf",
        "begin_date": begin, "end_date": end,
    ...
    try:
        js = _coops_get(COOPS_DATA, p_obs)
        if "error" not in js:
            data = js.get("data", [])
            if data:
                arr_t = np.array([
                    datetime.strptime(p["t"], "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc).timestamp()
                    for p in data if p.get("t") is not None
                ], dtype=float)
                arr_v = np.array([float(p["v"]) for p in data if p.get("v") is not None], dtype=float)
                _TIDE_DAY_SERIES[key] = (arr_t, arr_v)
                return arr_t, arr_v
    except Exception:
        pass

    _TIDE_DAY_SERIES[key] = (np.array([]), np.array([]))
    return _TIDE_DAY_SERIES[key]
...
def tide_at_time_from_series(arr_t: np.ndarray, arr_v: np.ndarray, dt: datetime) -> Tuple[Optional[float], Optional[str], Optional[str]]:
    ...
    if idx == 0:
        ...
        h = v0 + w * (v1 - v0)
    # slope using neighbors (~±6 min)
    i0 = max(0, idx-1); i1 = min(arr_t.size-1, idx)
    i0 = max(0, i0-1);   i1 = min(arr_t.size-1, i1+1)
...
def main():
    ...
    # Parse datetimes (handle Z/UTC and naïve)
    df["_dt"] = pd.to_datetime(df["Time"], errors="coerce", utc=True)  # treats 'Z' as UTC; local-naive assumed UTC
    if df["_dt"].isna().any():
        log("WARNING: Some 'Time' values could not be parsed; those rows may produce NaN outputs.")

    # Date-only (UTC) for daily fetches
    df["_date_utc"] = df["_dt"].dt.floor("D")

    # Vectorized nearest inlet
    lat_arr = df["Latitude"].to_numpy(float, copy=False)
    lon_arr = df["Longitude"].to_numpy(float, copy=False)
    names, dkm = nearest_inlet_vec(lat_arr, lon_arr)
    ...
    bins = [b for b in df["_bin"].dropna().unique()]

    # Parallel resolve GHCND stations per bin
    bin_to_ghcnd: Dict[Tuple[float,float], Optional[str]] = {}
    def _res_ghcnd(b):
        lat_b, lon_b = b
        st_g = find_nearest_ghcnd_station(lat_b, lon_b)
        return b, (None if not st_g else st_g.get("id"))
    with ThreadPoolExecutor(max_workers=min(8, max(2, len(bins)))) as ex:
        for fut in as_completed([ex.submit(_res_ghcnd, b) for b in bins]):
            b, sid = fut.result()
            bin_to_ghcnd[b] = sid

    # Parallel resolve CO-OPS currents & tide stations per bin
    bin_to_curr: Dict[Tuple[float,float], Optional[str]] = {}
    bin_to_tide: Dict[Tuple[float,float], Optional[str]] = {}
    def _res_coops(b):
        lat_b, lon_b = b
        st_c = nearest_coops_station(lat_b, lon_b, "currents")
        st_t = nearest_coops_station(lat_b, lon_b, "waterlevels")
        return b, (None if not st_c else (st_c.get("id") or st_c.get("station"))), (None if not st_t else (st_t.get("id") or st_t.get("station")))
    with ThreadPoolExecutor(max_workers=min(8, max(2, len(bins)))) as ex:
        for fut in as_completed([ex.submit(_res_coops, b) for b in bins]):
            b, curr_id, tide_id = fut.result()
            bin_to_curr[b] = curr_id
            bin_to_tide[b] = tide_id

    log(f"Tide stations resolved: {sum(x is not None for x in bin_to_tide.values())} / {len(bin_to_tide)} bins")

    df["_ghcnd_id"] = df["_bin"].map(bin_to_ghcnd)
    df["_curr_id"]  = df["_bin"].map(bin_to_curr)
    df["_tide_id"]  = df["_bin"].map(bin_to_tide)

    # Batch: GHCND per (station, date)
    ...
    df["tavg_c"] = tmp[0]; df["prcp_mm_prev_day"] = tmp[1]

    # Batch: Currents per (station, date)
    ...
    # Batch: Tides per (station, day) — fetch once/day series
    ...
    # Interpolate tide at exact sample times
    def map_tide(row):
        ...
        # dtv is already UTC (we parsed with utc=True)
        h, stage, intensity = tide_at_time_from_series(arr_t, arr_v, dtv.to_pydatetime())
